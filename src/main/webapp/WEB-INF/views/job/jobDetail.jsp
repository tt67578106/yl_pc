...
<script src="http://www.youlanw.com/static/js/youlanw.pc.js?v=5"></script>
...
<div class="page-wrapper bg-grey">
	...
	<!--/top-menu--->
    <div class="nav-g"><a href="${ctx}/">首页</a>&gt;<a href="${ctx}/zhaopin/">我要找工作</a>&gt;<a href="${ctx}/zhaopin_${jobBase.id}.html" class="red">${jobBase.title }</a>
    <c:if test="${not empty nextJobBase.id}">
    	<a href="${ctx}/zhaopin_${nextJobBase.id}.html" class="fr red mr10">[下一条]</a></div>
    </c:if>
    <c:if test="${empty nextJobBase.id}">
    	<a href="javascript:;" class="fr red mr10">[下一条]</a></div>
    </c:if>
    <!--基本信息-->
    <div class="w990">
     <input type="hidden" id="hid_type" value="${type }" />
        <div class="job-summary">
    	   ...
    	   <!-- /最近浏览记录 --> 		
        	<!--图集-->
			<div class="pic-slide" >
				<div class="bd <c:if test="${jobBase.jobConfig.isUrgency == 2}">seal-4</c:if><c:if test="${jobBase.jobConfig.isUrgency == 3}">seal-5</c:if>">
					<i></i>
					<ul class="job-list-each seal-1" >
				...
				<div class="hd">
					...
					<c:if test="${fn:length(imageList)>4}">
						 <a href="${ctx}/company/companyDetailAlbum?id=${jobBase.company.id }" target="_blank">
							<span class="link">
								<p>更多图片</p>
								<p>（${fn:length(imageList)-1}张）</p>
							</span>
						</a>
					</c:if>
				</div>
           ...
           <div class="sum">
                ...
                <p><label class="name-g">更新时间</label>${jobBase.updatetimeString}</p>
                <p class="num">
                ...
                <p class="tags-g mt15">
                 <c:forEach items="${jobBase.jobLabel}" var="label">
                  ...
                  <c:if test="${jobBase.cooperationType eq 1 && jobBase.jobConfig.isRecruitment eq 0}"><p class="sign"><a data-id="${jobBase.id }" class="btn signup-layer-trigger btn-signup fl" >申请面试</a></c:if>
                  <c:if test="${jobBase.cooperationType ne 1 || jobBase.jobConfig.isRecruitment ne 0}"> <p class="sign"><a data-id="${jobBase.id }" class="btn signup-layer-trigger btn-signup fl" >立即预约</a></c:if>
                	...
                	<div class="top-fixed">
                        <ul class="tab-g-title nav">
                            ...
                            <li><a href="#pay-info">福利待遇</a></li>
                 			<c:if test="${jobBase.cooperationType eq 1 && jobBase.jobConfig.isRecruitment eq 0}"><p class="sign"><a data-id="${jobBase.id }" class="btn signup-layer-trigger " >申请面试</a></c:if>
                  			<c:if test="${jobBase.cooperationType ne 1 || jobBase.jobConfig.isRecruitment ne 0}"> <p class="sign"><a data-id="${jobBase.id }" class="btn signup-layer-trigger" >立即预约</a></c:if>
                        </ul>
                    </div>
                </div>
            </div>
      ...
      <!---->
		...
		<div class="w990 mt20">
			...
			<div class="tab-each show">
				<c:forEach items="${newJobs }" var="hotJob">
    ...
    <div class="div_signup hide">
        ...
        <div class="form-g-medium fl mt0">
           ...
           <p class="tip name_err_tip"></p>
			...
			<c:if test="${jobBase.cooperationType eq 1 && jobBase.jobConfig.isRecruitment eq 0}">
           ...
           <!--民族-->
            ...
            <c:if test="${jobBase.cooperationType ne 1 || jobBase.jobConfig.isRecruitment ne 0}"> 
